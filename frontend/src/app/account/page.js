...
export default function Account() {
    ...
    useEffect(() => {
        ...
        // Ensure payment always exist
        // parsed.payment = parsed.payment || { cardNumber: "", name: "", expiryDate: "", cvc: "" };
    ...
    //     if (!storedId) return router.replace("/login");

    //     async function fetchUser() {
    //         try {
    //             const res = await fetch(`http://localhost:8080/api/customers/${storedId}`);
    ...
    //             setUser(data);
    //             setOriginalUser(JSON.parse(JSON.stringify(data))); // Deep clone
    //             setId(storedId);
